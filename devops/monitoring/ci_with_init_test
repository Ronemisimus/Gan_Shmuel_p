...
def gitWebHook():
	...
	if branch == 'weight' or branch == 'provider':
		...
		os.system('ls -la') #DEBUG
		curr_port = os.environ['PORT'] #saves the staging port ##############################
		os.environ['PORT'] = 8082 #use test port for the initial run ###########################
		os.system('docker-compose -p {}-{} up -d'.format(environment, branch)) 
		#TODO: run E2E test (given by the other teams)
		os.chdir('/ci-test')
